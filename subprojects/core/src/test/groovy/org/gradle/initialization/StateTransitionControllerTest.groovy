/*
 * Copyright 2021 the original author or authors.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.gradle.initialization

import org.gradle.test.fixtures.concurrent.ConcurrentSpec

class StateTransitionControllerTest extends ConcurrentSpec {
    enum TestState implements StateTransitionController.State {
        A, B, C
    }

    def "transitions when in from state"() {
        def action = Mock(Runnable)
        def controller = new StateTransitionController(TestState.A)

        when:
        controller.maybeTransition(TestState.A, TestState.B, action)

        then:
        1 * action.run()
        0 * _
    }

    def "does not transition when already in to state"() {
        def action = Mock(Runnable)
        def controller = new StateTransitionController(TestState.A)
        controller.maybeTransition(TestState.A, TestState.B) {}

        when:
        controller.maybeTransition(TestState.A, TestState.B, action)

        then:
        0 * _
    }

    def "does not transition when has already transitioned from to state"() {
        def action = Mock(Runnable)
        def controller = new StateTransitionController(TestState.A)
        controller.maybeTransition(TestState.A, TestState.B) {}
        controller.maybeTransition(TestState.B, TestState.C) {}

        when:
        controller.maybeTransition(TestState.A, TestState.B, action)

        then:
        0 * _
    }

    def "action cannot attempt to transition state while not in from state"() {
        def action = Mock(Runnable)
        def controller = new StateTransitionController(TestState.A)

        when:
        controller.maybeTransition(TestState.B, TestState.C, action)

        then:
        def e = thrown(IllegalStateException)
        e.message == "Can only transition to state C from state B however currently in state A."
    }

    def "action cannot attempt to transition state while already transitioning"() {
        def action = Mock(Runnable)
        def controller = new StateTransitionController(TestState.A)

        when:
        controller.maybeTransition(TestState.A, TestState.B, action)

        then:
        def e = thrown(IllegalStateException)
        e.message == "Cannot transition to state B as already transitioning to state B."

        1 * action.run() >> {
            controller.maybeTransition(TestState.A, TestState.B, { })
        }
        0 * _
    }

    def "another thread cannot attempt to transition state"() {
        def controller = new StateTransitionController(TestState.A)

        when:
        async {
            start {
                controller.maybeTransition(TestState.A, TestState.B) {
                    instant.transitioning
                    thread.block()
                }
            }
            start {
                thread.blockUntil.transitioning
                controller.maybeTransition(TestState.A, TestState.B) {
                }
            }

        }

        then:
        def e = thrown(IllegalStateException)
        e.message == "Another thread is currently transitioning state."
    }
}
